...
export default function ChatInput({
  ...
  return (
    <form onSubmit={handleSubmit} className="p-3 border-t border-slate-800/80 bg-slate-950 shrink-0">
      {/* Preview Thumbnail */}
      {attachmentUrl && (
        <div className="flex items-center gap-2 mb-2 p-1.5 bg-slate-900 border border-slate-800 rounded-lg max-w-[200px] relative group select-none">
          <img src={attachmentUrl} alt="Preview" className="w-10 h-10 object-cover rounded" />
          <span className="text-[10px] text-slate-400 truncate max-w-[110px] font-medium">Captura pronta</span>
          <button
            type="button"
            onClick={() => setAttachmentUrl(null)}
            className="absolute -top-1.5 -right-1.5 p-0.5 bg-rose-950 text-rose-200 border border-rose-800 rounded-full hover:bg-rose-900 cursor-pointer transition-all"
            title="Remover anexo"
          >
      ...
      <div className="relative flex items-center gap-2">
        <button
          ...
          className="p-2.5 hover:bg-slate-900 border border-slate-800/60 rounded-xl text-slate-400 hover:text-white transition-all cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/50 shrink-0"
